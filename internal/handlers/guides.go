...
// generatedGuidesJSON is the canonical guide content, generated by
// `make genguides` (cmd/genguides). The CLI writes guides_data.gen.json
// next to this file; that file is committed and embedded here so the
// runtime binary contains the data with no file-IO cost.
//
// Kept as a string-typed embed (rather than `[]byte`) so an empty
// file or a missing-at-build-time-but-present-as-zero-bytes scenario
// gives us a single observable signal: `generatedGuidesJSON == ""`.
// In that case we fall through to `staticGuides()` below — the
// pre-generation hand-written set, kept as a defensive fallback so
// local dev without a generated file still serves something.
//
//go:embed guides_data.gen.json
var generatedGuidesJSON string

// generatedGuide is the JSON shape produced by cmd/genguides. It mirrors
// `Guide` minus the runtime-derived CTAText/CTAURL fields, which we add
// at load time using the appURL from server config.
...
// NewGuidesHandler creates a new GuidesHandler. Loads guide content from
// the embedded `guides_data.gen.json` (produced by `make genguides`) when
// available, falling back to the legacy hand-written `staticGuides()`
// when the embed is empty (local dev without a generated artefact, or
// a pre-PR-3-of-#30 binary).
//
// The fallback is deliberate: it's the only thing protecting local dev
// from a "boots with no guides" state if a developer forgets to run
// the generator before testing. Production binaries always carry the
// generated JSON because it's checked in.
func NewGuidesHandler(appURL string) *GuidesHandler {
	guides, source := loadGuides(appURL)
	bySlug := make(map[string]*Guide, len(guides))
	for i := range guides {
		bySlug[guides[i].Slug] = &guides[i]
	}
	slog.Info("guides loaded", "source", source, "count", len(guides))
	return &GuidesHandler{guides: guides, bySlug: bySlug}
}

// loadGuides resolves the guide content + source label. Splits the
// embed-vs-static decision out of NewGuidesHandler so tests can poke at
// the boundaries independently if needed.
//
// Order of preference:
//  1. Embedded `guides_data.gen.json` (the post-PR-2-of-#30 path).
//  2. `staticGuides()` (the pre-generation hand-written set).
//
// Returns the guide slice + a short label ("generated" or "static") for
// the boot-time slog so an operator can see at a glance which set is
// serving traffic.
func loadGuides(appURL string) ([]Guide, string) {
	if strings.TrimSpace(generatedGuidesJSON) != "" {
		var raw []generatedGuide
		err := json.Unmarshal([]byte(generatedGuidesJSON), &raw)
		switch {
		case err != nil:
			// Malformed embed must NOT silently serve the legacy set.
			// A real production deploy with a corrupt embed should be
			// a deploy-time bug, not a stealth regression.
			slog.Error("failed to parse embedded guides_data.gen.json — falling back to staticGuides()", "error", err)
		case len(raw) == 0:
			// Well-formed but empty array — the generator produced
			// no guides. Log informationally rather than as an error;
			// staticGuides() is the right thing to serve until the
			// generator is re-run with content.
			slog.Warn("embedded guides_data.gen.json is empty — falling back to staticGuides()")
		default:
			out := make([]Guide, 0, len(raw))
			for _, g := range raw {
				out = append(out, Guide{
					Slug:             g.Slug,
					Title:            g.Title,
					PersonaName:      g.PersonaName,
					PersonaSpecialty: g.PersonaSpecialty,
					Destination:      g.Destination,
					Country:          g.Country,
					Theme:            g.Theme,
					Excerpt:          g.Excerpt,
					Content:          g.Content,
					CTAText:          deriveCTAText(g.Destination, g.PersonaName),
					CTAURL:           appURL,
				})
			}
	...
	return staticGuides(appURL), "static"
}

// deriveCTAText constructs the call-to-action label for a guide.
// Format mirrors the legacy hand-written CTAText shape:
//
//	"Plan your <destination> trip with <persona>"
//
// We deliberately don't theme-vary the suffix (the old hand set had
// "food trip", "nights", "whisky tour", etc — all bespoke). A
// consistent template is more predictable for the marketing site
// integration and for the dynamic guide rollouts in PR 3+ of #30.
//
// If either input is empty, we degrade gracefully — empty destination
// would produce "Plan your trip with X" which is still readable, and
// empty persona produces "Plan your X trip" without trailing fragment.
func deriveCTAText(destination, personaName string) string {
	switch {
	case destination != "" && personaName != "":
		return fmt.Sprintf("Plan your %s trip with %s", destination, personaName)
	case destination != "":
		return fmt.Sprintf("Plan your %s trip", destination)
	case personaName != "":
		return fmt.Sprintf("Plan your trip with %s", personaName)
	default:
...
func staticGuides(appURL string) []Guide {
	return []Guide{
		...
		{Slug: "sydney-adventure", Title: "Sydney Adventure Guide by Liam", PersonaName: "Liam", PersonaSpecialty: "Outdoor Adventure", Destination: "Sydney", Country: "AU", Theme: "adventure", Excerpt: "Liam knows every coastal walk, surf break, and hidden swimming hole around Sydney.", Content: "Sydney is built for outdoor adventure — the harbour, the coastline, the bush, and the ocean are all within easy reach of the CBD.\n\nThe Bondi to Coogee coastal walk is a must — 6km of clifftop trails, ocean pools, and beaches. Start early at Icebergs pool for a swim, then walk south.\n\nFor surfing, Manly Beach has consistent waves for intermediate surfers. The ferry from Circular Quay is itself an adventure — 30 minutes through the harbour with Opera House views.\n\nThe Blue Mountains are 90 minutes by train. The Grand Canyon Walk near Blackheath is a 6.3km loop descending into a fern-filled canyon that feels like a lost world.", CTAText: "Plan your Sydney adventure with Liam", CTAURL: appURL},
	}
}
