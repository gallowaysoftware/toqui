...
func TestParseEnvelope(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "valid envelope",
			input: `{"from":"sender@example.com","to":["recipient@toqui.app"]}`,
			from:  "sender@example.com",
			to:    []string{"recipient@toqui.app"},
		},
		{
			name:  "multiple recipients",
			input: `{"from":"sender@example.com","to":["a@toqui.app","b@toqui.app"]}`,
			from:  "sender@example.com",
			to:    []string{"a@toqui.app", "b@toqui.app"},
		},
		{
			name:    "empty string",
			input:   "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			env, err := parseEnvelope(tt.input)
			if tt.wantErr {
				if err == nil {
			...
			if env.From != tt.from {
				t.Errorf("from = %q, want %q", env.From, tt.from)
			}
			if len(env.To) != len(tt.to) {
				t.Fatalf("to length = %d, want %d", len(env.To), len(tt.to))
			}
			for i, to := range env.To {
				if to != tt.to[i] {
					t.Errorf("to[%d] = %q, want %q", i, to, tt.to[i])
				}
			}
		})
	}
