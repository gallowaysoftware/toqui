"use client";

import { useParams } from "next/navigation";
import { ChatContainer } from "@/components/chat/ChatContainer";
import { useTrip } from "@/lib/hooks/useTrips";
import { TripStatus } from "@/gen/toqui/v1/trip_pb";

export default function ChatPage() {
  const { tripId } = useParams<{ tripId: string }>();
  const { trip, isLoading } = useTrip(tripId);

  const mode = trip?.status === TripStatus.ACTIVE ? "companion" : "planning";

  if (isLoading) {
    return (
      <div className="h-screen flex items-center justify-center">
        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" />
      </div>
    );
  }

  return (
    <div className="h-screen flex flex-col">
      <header className="bg-white border-b border-gray-200 px-4 py-3 flex-shrink-0 flex items-center justify-between">
        <h1 className="text-lg font-semibold">
          {mode === "companion" ? "Travel Companion" : "Trip Planning"}
        </h1>
        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
          mode === "companion" ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"
        }`}>
          {mode === "companion" ? "Traveling" : "Planning"}
        </span>
      </header>
      <ChatContainer tripId={tripId} mode={mode} />
    </div>
  );
}
