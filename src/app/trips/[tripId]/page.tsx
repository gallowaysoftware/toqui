...
export default function TripDetailPage() {
  ...
  const handleStartTrip = () => {
    updateTrip.mutate(
      { id: tripId, status: TripStatus.ACTIVE },
      { onSuccess: () => setStatusMessage("Trip started! You're now in travel mode.") },
    );
  };

  const handleCompleteTrip = () => {
    updateTrip.mutate(
      { id: tripId, status: TripStatus.COMPLETED },
      { onSuccess: () => setStatusMessage("Trip marked as completed.") },
    );
  };
    ...
    <div className="min-h-screen bg-[var(--color-surface-secondary)]">
      ...
      {statusMessage && (
        <div
          ...
          aria-live="polite"
        >
          <div className="bg-[var(--color-success-bg,var(--color-status-active-bg))] text-[var(--color-success-text,var(--color-status-active-text))] text-sm font-medium px-4 py-2.5 rounded-lg">
            {statusMessage}
          </div>
        </div>
      )}
