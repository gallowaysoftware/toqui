...
toqui self-hosted: the stack for running toqui on the home server against
the local LLM fleet. toqui is a plain OpenAI-compatible client — it points
at the fleet front (hum `:9000`; the fleet's own docs live in the vibe
repo, `docs/design/topology.md`), picks catalog model ids, and shares warm
models with every other client. Layered on top of the fleet, not coupled
to it — which is why this stack lives HERE with the app, not in the fleet
repo (moved from vibe:deploy/toqui, 2026-07-31). The backend builds from
the archived `toqui-backend` repo (read-only but clonable). It deploys and upgrades on its own lifecycle; all its data
...
BEST call-to-call. **Rule (fleet house rule — vibe:docs/design/topology.md UC3): within one cell, all tiers map to
