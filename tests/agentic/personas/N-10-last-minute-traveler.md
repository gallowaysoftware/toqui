...
2. **Skip to companion mode**: After the trip is created, set the trip status to ACTIVE to enter companion mode. This tests the companion-first flow where a user skips the planning phase entirely because they are already en route.
...
- The AI should NOT try to create a detailed multi-day itinerary in planning mode. If the agent sets the trip to active/companion mode, the AI should respond in companion mode style: immediate, location-aware, practical.
