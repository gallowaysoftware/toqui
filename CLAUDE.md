...
In production web, a runtime `config.json` is generated by the Docker container entrypoint from environment variables, allowing config injection without rebuilding the static bundle.

## Deployment

See [DEPLOYMENT.md](DEPLOYMENT.md) at the repo root. Three patterns are documented: docker-compose on a single host, Fly.io, and Render.

### iOS / Android
`pnpm build:ios` and `pnpm build:android` produce native bundles via EAS Build. Self-hosters who want to ship to the App Store / Play Store provision their own EAS account.
